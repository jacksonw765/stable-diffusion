...
def realesrgan4x(executable: str, input: str, output: str):
    ...
    final_output = Image.open(output)
    final_output = final_output.resize((int(final_output.size[0] * 3), int(final_output.size[1] * 3)),
                                       get_resampling_mode())
    final_output.save(output)


def realesrgan8x(executable: str, input: str, output: str):
    process = subprocess.Popen([
        executable,
        '-i',
        input,
    ...
    final_output = Image.open(output)
    final_output = final_output.resize((int(final_output.size[0] * 6), int(final_output.size[1] * 6)),
                                       get_resampling_mode())
    final_output.save(output)
...
def text2img2(opt: Options):
    ...
    for init_img_number in generated:
        ...
        for _ in trange(opt.passes, desc="Passes"):
            realesrgan2x(opt.realesrgan, os.path.join(sample_path, f"{base_filename}.png"),
