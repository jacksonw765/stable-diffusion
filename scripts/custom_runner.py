...
def main():
    ...
    parser.add_argument(
        '--upscale',
        type=str,
        help='perform upscaler, specify file'
    )
    parser.add_argument(
        '--id',
        type=str,
    ...
    parser.add_argument(
        '--upscale_dif',
        action='store_true',
        help='perform diffusion upscaler'
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    if opt.id is not None:
        if opt.upscale is not None and opt.prompt is None:
            upscale_image(opt)
        elif opt.prompt is not None:
            text2img2(opt)
            upscale_image(opt)
        else:
                    ...
                    for n in trange(1, desc="Sampling"):
                        for prompts in tqdm(data, desc="data"):
                            ...
                            for x_sample in x_checked_image_torch:
                                ...
                                output_path = os.path.join(sample_path, f"{_id}.png")
...
# TODO
# def upscale_dif_image(opt: Options, sample_path):
#     batch_size = 1
