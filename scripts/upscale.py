...
def realesrgan4x(executable: str, input: str, output: str):
    ...
    final_output = final_output.resize((int(final_output.size[0] * 2), int(final_output.size[1] * 2)),
                                       get_resampling_mode())
...
def grid_slice(source, overlap, og_size, maximize=False):
    ...
    if maximize == True:
        ...
        coordinates, new_size = grid_coords(source.size, og_size,
                                            overlap)  # re-do the coordinates with the new canvas size
...
def addalpha(im, mask):
    ...
    im = Image.merge('RGBA',
                     [imr, img, imb, mma])  # we want the RGB from the original, but the transparency from the mask
    return (im)
...
def upscale(opt: Options):
    start = datetime.now()
    base_filename = opt.file
    outpath = opt.outdir
    sample_path = os.path.join(outpath, "samples")
    realesrgan4x(opt.realesrgan, os.path.join(sample_path, f"{base_filename}.png"),
                 os.path.join(sample_path, f"{base_filename}u.png"))
    source_image = Image.open(os.path.join(sample_path, f"{base_filename}.png"))
    og_size = source_image.size  # grab og size

    base_filename = f"{opt.file}u"
    source_image = Image.open(os.path.join(sample_path, f"{base_filename}.png"))
    slices, _ = grid_slice(source_image, 64, og_size, False)  # gobig_overlap = 64
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    config = OmegaConf.load(f"{opt.config}")
    model = load_model_from_config(config, f"{opt.ckpt}")
    model = model.to(device)
    sampler = DDIMSampler(model) #DDIMSampler
